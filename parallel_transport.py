...
keys = [0,    # Keyframe 0. Draw origin + gridlines.
        ...
        18-2,   # Keyframe 5. Traveler's journey, but it's a square (zoomed out)
        19-2+2,   # Keyframe 6. Undo traveler's journey.  TODO
        ...
        32,   # Keyframe 10. Draw lattitude line on cone.
        35    # DONE.
        ]
...
def to_cone(u, point, cutout_interval=(np.pi, 3*np.pi/2)):
    ...
    sigma = ease_out(slash(ease_inout_inverse(u), 0.6))
...
def main():
    ...
    while run:
        ...
        version = 2  # makeshift, for now.  1 = "third quad missing", 2 = 3/4 of grid missing,
        #                                     # 3 = still 3/4 missing, but spiral curve, 4 = still 3/4 missing, but circle curve
        # show_arrows = False  # show the parallel transport arrows across whichever path we're drawing
        # arrow_default = 100. * np.array([0., 1., 0.])
        # arrow_sparsity = 50 if version > 2 else 5  # every ? points should there be an arrow?
        #
        # show_frenet = True
        #
        # if frame == 7:
        ...
        #     else:
        #         pygame.draw.lines(window, np.array([76, 0, 255]), False,
        #                           [np.array([width / 2, 0]), np.array([width / 2, height / 2]),
        #                            np.array([width, height / 2])], width=10)
        #
        #     # Redraw origin
        #     radius = 10
        #     center2d = A(shift(world_to_plane(np.array([0, 0, 0]))))
        ...
        #     spacing = 100.0
        #     grid_width, grid_height = width * 5, height * 5
        #     for x in np.arange(-grid_width / 2 if version == 1 else 0, grid_width / 2, spacing):
        #         start_y, finish_y = grid_height / 2, -grid_height / 2 if version == 1 else 0
        #         start_pt, finish_pt = world_to_plane(np.array([x, start_y, 0])), world_to_plane(
        #             np.array([x, finish_y, 0]))
        #         pygame.draw.line(window, white * 0.5, *A_many(shift_many([start_pt, finish_pt])), width=2)
        #     for y in np.arange(-grid_height / 2 if version == 1 else 0, grid_height / 2, spacing):
        #         start_x, finish_x = -grid_width / 2 if version == 1 else 0, grid_width / 2
        #         start_pt, finish_pt = world_to_plane(np.array([start_x, y, 0])), world_to_plane(
        #             np.array([finish_x, y, 0]))
        #         pygame.draw.line(window, white * 0.5, *A_many(shift_many([start_pt, finish_pt])), width=2)
        #     # Redraw traveler
        ...
        #     if version == 2:
        #         pts = get_many_line_pts(u, np.array([+7*spacing, 5*spacing, 0]), np.array([-5*spacing, -10.5*spacing, 0]), du=0.005)
        #     elif version == 3:
        #         pts = get_spiral_pts(u, scaling=100., r0=100., du=0.001)
        #     elif version == 4:
        #         pts = get_spiral_pts(u, scaling=0., r0=500., theta_max=np.pi/2, du=0.001)
        #
        #     radius /= 2
        #     pts, cut_indices, R_list = through_portals(pts, cutout_interval=(np.pi/2, 2*np.pi) if version > 1 else (np.pi, 3*np.pi/2))
        ...
        #
        #     # Draw arrows if show_arrows
        #     if show_arrows:
        #         for i in range(1, len(pts), arrow_sparsity):
        #             R = None
        #             for k, cut in enumerate(cut_indices):
        #                 if i < cut:
        #                     R = R_list[k-1]
        #                     break
        #
        #             pt0, pt1 = pts[i], pts[i] + (R @ arrow_default)
        #             if min(pt1[0], pt1[1]) > 0:
        #                 pygame.draw.lines(window, traveler_blue, False, A_many(world_to_plane_many([pt0, pt1])), width=5)
        ...
        if frame == 9:
            ...
            # VERSION >1 ——— THREE QUADRANT CUTOUT
            if version > 1:
                cutout_interval = (np.pi/2, 2 * np.pi)
                ...
                for x in np.arange(-spacing * n, spacing * n, spacing):
                    for y in np.arange(-spacing * n, spacing * n, spacing):
                        ...
                        if angle <= cutout_interval[0] or angle >= cutout_interval[1]:
                            domain_point0 = A(world_to_plane(to_cone(u, np.array([x, y, 0]), cutout_interval=cutout_interval)))
                            angle = np.arctan2(y, x + spacing) % (2 * np.pi)
                            col = white * (1-u + (u * 0.5))
                            if angle <= cutout_interval[0] or angle >= cutout_interval[1]:
                                domain_point1 = A(world_to_plane(to_cone(u, np.array([x + spacing, y, 0]), cutout_interval=cutout_interval)))
                                pygame.draw.line(window, col, domain_point0, domain_point1, width=1)
                            angle = np.arctan2(y + spacing, x) % (2 * np.pi)
                            if angle <= cutout_interval[0] or angle >= cutout_interval[1]:
                                domain_point2 = A(world_to_plane(to_cone(u, np.array([x, y + spacing, 0]), cutout_interval=cutout_interval)))
                                pygame.draw.line(window, col, domain_point0, domain_point2, width=1)

                # # DEMO PATH 2
                # # spacing *= 2
                # pts = get_many_line_pts(1, np.array([+7*spacing, 5*spacing, 0]), np.array([-5*spacing, -10.5*spacing, 0]), du=0.005)
                # if version == 3:
                #     pts = get_spiral_pts(1, scaling=100., r0=100., du=0.001)
                # elif version == 4:
                #     pts = get_spiral_pts(1, scaling=0., r0=500., theta_max=np.pi/2, du=0.001)
                #
                # pts, cut_indices, R_list = through_portals(pts, cutout_interval)
                # coned_pts = to_cone_many(u, pts, cutout_interval)
                # for k in range(len(cut_indices) - 1):
                #     segment = coned_pts[cut_indices[k]:cut_indices[k+1]]
                #     if len(segment) > 2:
                #         pygame.draw.lines(window, trace_line_col, False, A_many(world_to_plane_many(segment)), width=5)
                #
                # # Draw parallel transport arrows if show_arrows
                # if show_arrows:
                #     for i in range(1, len(pts), arrow_sparsity):
                #         R = None
                #         for k, cut in enumerate(cut_indices):
                ...
                #         pt0, pt1 = pts[i], pts[i] + (R @ arrow_default)
                #         if min(pt1[0], pt1[1]) > 0:
                #             line_ = world_to_plane_many(to_cone_many(u, [pt0, pt1], cutout_interval=cutout_interval))
                #             pygame.draw.lines(window, traveler_blue, False, A_many(line_), width=5)

            # # VERSION 1 ——— THIRD QUADRANT CUTOUT
            # if version == 1:
            #     # Draw cone lines (the ones which will actually move around to display the transformation)
            #     radius = 3
            #     n = 10
            ...
            #     radius /= 2
            #     pts, cut_indices, R_list = through_portals(pts)
            #     coned_pts = to_cone_many(u, pts)
            #     for k in range(len(cut_indices) - 1):
            #         segment = coned_pts[cut_indices[k]:cut_indices[k+1]]
            #         if len(segment) > 2:
            #             pygame.draw.lines(window, trace_line_col, False, A_many(world_to_plane_many(segment)), width=5)
            ...
            #
            #             pt0, pt1 = pts[i], pts[i] + (R @ arrow_default)
            #             line_ = world_to_plane_many(to_cone_many(u, [pt0, pt1]))
            #             pygame.draw.lines(window, traveler_blue, False, A_many(line_), width=5)
